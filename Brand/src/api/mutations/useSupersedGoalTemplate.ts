...
export function useSupersedGoalTemplate() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api.post<GoalTemplateVersionDto>(
        `/v1/admin/brand-studio/goal-templates/${id}/supersede`,
        { rowVersion, promptText: newPromptText, notes },
      ).then((r) => r.data),

    onSuccess: (_, { goal }) => {
      qc.invalidateQueries({ queryKey: bsQk.goalTemplates.list(goal) })
      qc.invalidateQueries({ queryKey: bsQk.goalTemplates.active(goal) })
      toast.success('Goal template superseded.')
    },
